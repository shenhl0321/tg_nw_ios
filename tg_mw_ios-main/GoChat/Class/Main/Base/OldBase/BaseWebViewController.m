...
@interface BaseWebViewController ()<WKScriptMessageHandler, WKUIDelegate, WKNavigationDelegate, BusinessListenerProtocol>
...
//        [backBtn setImage:[UIImage imageNamed:@"com_nav_ic_back_black_normal"] forState:UIControlStateNormal];
//        [backBtn setImage:[UIImage imageNamed:@"com_nav_ic_back_black_pressed"] forState:UIControlStateHighlighted];
//        [backBtn addTarget:self action:@selector(gotoBack_Present) forControlEvents:UIControlEventTouchUpInside];
...
//        NSString *jSString = @"var meta = document.createElement('meta'); meta.setAttribute('name', 'viewport'); meta.setAttribute('content', 'width=device-width'); document.getElementsByTagName('head')[0].appendChild(meta);";
        ...
        //以下代码适配文本大小user-scalable=no" />
        NSString *jSString = [[NSString alloc] init];
        if (self.isNoZoom){// , user-scalable=no
            jSString = @"var meta = document.createElement('meta'); meta.setAttribute('name', 'viewport'); meta.setAttribute('content', 'width=device-width', initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0); document.getElementsByTagName('head')[0].appendChild(meta);";
        }else{
            jSString = @"var meta = document.createElement('meta'); meta.setAttribute('name', 'viewport'); meta.setAttribute('content', 'width=device-width'); document.getElementsByTagName('head')[0].appendChild(meta);";
        }
        //用于进行JavaScript注入
        ...
        if(self.type == WEB_LOAD_TYPE_NORMAL)
        {
            [self.webView loadHTMLString:self.htmlString baseURL:[NSURL fileURLWithPath:[[NSBundle mainBundle] bundlePath]]];
        }
        if(self.type == WEB_LOAD_TYPE_URL)
        {
            if(!IsStrEmpty(self.urlStr))
                [self.webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:self.urlStr]]];
        }
        if(self.type == WEB_LOAD_TYPE_TAB_EX_URL)
        {
