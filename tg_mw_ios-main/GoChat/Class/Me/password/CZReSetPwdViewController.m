...
- (void)viewDidLoad {
    [super viewDidLoad];
    if (self.hasPwd) {
        self.title = @"重置登录密码".lv_localized;
        [self.customNavBar setTitle:@"重置登录密码".lv_localized];
    }else{
        self.title = @"设置登录密码".lv_localized;
    ...
    [[TelegramManager shareInstance] changeLoginPaswordWithParams:[CZCommonTool dictionaryToJson:paramsDic] resultBlock:^(NSDictionary *request, NSDictionary *response, id obj){
        ...
        if(obj){
            if ([obj isKindOfClass:[NSDictionary class]]) {
                if (self.hasPwd) {
                    [UserInfo showTips:self.view des:@"重置登录密码成功".lv_localized];
                }else{
                    [UserInfo showTips:self.view des:@"设置登录密码成功".lv_localized];
                }
                [self.navigationController popToRootViewControllerAnimated:YES];
            }else if([obj isKindOfClass:[NSString class]]){
                [UserInfo showTips:self.view des:obj];
            }
        }else{
...
- (NSString *)getFieldStrWithTag:(NSInteger)tag{
    for (UITableViewCell *cell in self.mainTableview.visibleCells) {
        if (cell.tag == tag) {
            if ([cell isMemberOfClass:[CZVerifiyTableViewCell class]]) {
                CZVerifiyTableViewCell *cellLim = (CZVerifiyTableViewCell *)cell;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSString *ID = @"CZVerifiyTableViewCell";
    CZVerifiyTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (cell == nil) {
        cell = [[[NSBundle mainBundle] loadNibNamed:@"CZVerifiyTableViewCell" owner:nil options:nil] firstObject];
    }
