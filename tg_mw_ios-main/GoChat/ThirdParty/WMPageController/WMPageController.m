...
- (void)wm_addMenuView {
    ...
    if (self.titleFontNameSelected) {
        menuView.selectedFontName = self.titleFontNameSelected;
    }
...
- (BOOL)wm_layoutChildViewControllers {
...
//    [self.menuView addRoundedCorners:UIRectCornerBottomLeft|UIRectCornerBottomRight radius:16];
}

- (CGFloat)wm_calculateItemWithAtIndex:(NSInteger)index {
    NSString *title = [self titleAtIndex:index];
//    UIFont *titleFont = self.titleFontName ? [UIFont fontWithName:self.titleFontName size:self.titleSizeNormal] : [UIFont fontWithName:self.titleFontNameSelected size:self.titleSizeSelected];
    UIFont *titleFont = [UIFont fontWithName:self.titleFontNameSelected size:self.titleSizeSelected];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if(![self wm_layoutChildViewControllers]){
        return;
    }
...
- (NSString *)menuView:(WMMenuView *)menu titleFontForState:(WMMenuItemState)state atIndex:(NSInteger)index{
    switch (state) {
        case WMMenuItemStateSelected: return self.titleFontNameSelected;
        case WMMenuItemStateNormal: return self.titleFontName;
    }
}
