    ...
    if (!animation) {
        if (self.selected) {
            if (self.selectedFontName) {
                self.font = [UIFont fontWithName:self.selectedFontName
            ...
            if (self.normalFontName) {
                self.font = [UIFont fontWithName:self.normalFontName
...
- (void)rateChange {
    if (_gap > 0.000001) {
        ...
        NSLog(@"self.selectedFontName: %@",self.selectedFontName);
        NSLog(@"self.normalFontName: %@",self.normalFontName);
        if (self.selected) {
            [self setFont:[UIFont fontWithName:self.selectedFontName size:self.selectedSize]];
        }else{
            [self setFont:[UIFont fontWithName:self.normalFontName size:self.normalSize]];
        }
    }
}
