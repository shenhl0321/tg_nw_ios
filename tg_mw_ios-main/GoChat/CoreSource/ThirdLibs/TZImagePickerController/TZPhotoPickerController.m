...
- (void)doneButtonClick {
    ...
    if (tzImagePickerVc.selectedModels.count <= 0 || tzImagePickerVc.onlyReturnAsset) {
        [self didGetAllPhotos:photos assets:assets infoArr:infoArr];
    }else if(photos.count == tzImagePickerVc.selectedModels.count){
...
- (void)pushPhotoPrevireViewController:(TZPhotoPreviewController *)photoPreviewVc needCheckSelectedModels:(BOOL)needCheckSelectedModels {
    ...
    [photoPreviewVc setEditButtonClickBlock:^(UIImage * photo) {
        TZImagePickerController *tzImagePickerVc = (TZImagePickerController *)self.navigationController;
//        if ([tzImagePickerVc.pickerDelegate respondsToSelector:@selector(imagePickerController:didFinishEditImage:)]) {
//            [tzImagePickerVc.pickerDelegate imagePickerController:tzImagePickerVc didFinishEditImage:photo];
//        }
        if (tzImagePickerVc.didFinishEditPhotosHandle) {
            tzImagePickerVc.didFinishEditPhotosHandle(photo);
