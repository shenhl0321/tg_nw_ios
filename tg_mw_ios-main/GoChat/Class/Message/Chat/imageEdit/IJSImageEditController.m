        ...
        [weakSelf _completeCallback:^(UIImage *image) {
            ...
            if (weakSelf.completeEditImageCallBlock)
            {
                [IJSVideoManager saveImageToSandBoxImage:image completion:^(NSURL *outputPath, NSError *error) {
                    if (error)
                    {
                        UIAlertController *alertView = [UIAlertController alertControllerWithTitle:@"警告".lv_localized message:[NSString stringWithFormat:@"%@", error] preferredStyle:(UIAlertControllerStyleActionSheet)];
...
//            [weakSelf dismissViewControllerAnimated:YES completion:nil];
    ...
    self.toolsView.panButtonBlock = ^(UIButton *button) {
        ...
        weakSelf.placeholderToolView.frame = CGRectMake(0, JSScreenHeight - ToolBarMarginBottom -34 - ColorButtonViewWidth, JSScreenWidth, ColorButtonViewWidth);
        if (IJSGiPhoneX)
        {
            weakSelf.placeholderToolView.frame = CGRectMake(0, JSScreenHeight -IJSGTabbarSafeBottomMargin - ToolBarMarginBottom - ColorButtonViewWidth - 34, JSScreenWidth, ColorButtonViewWidth);
        }
    ...
    self.toolsView.mosaicButtonBlock = ^(UIButton *button) {
        ...
        if (IJSGiPhoneX)
        {
             weakSelf.placeholderToolView.frame = CGRectMake(0, JSScreenHeight - IJSGTabbarSafeBottomMargin - ToolBarMarginBottom - IJSImageMosaicButtonHeight -34, JSScreenWidth, IJSImageMosaicButtonHeight);
        }
        else
        {
            weakSelf.placeholderToolView.frame = CGRectMake(0, JSScreenHeight - ToolBarMarginBottom - 34 - IJSImageMosaicButtonHeight, JSScreenWidth, IJSImageMosaicButtonHeight);
        }
        [weakSelf _hiddenPlaceholderToolView:isHidden];
        weakSelf.mosaicToolView.hidden = isHidden;
        if (!button.selected) //选中
         {
             [weakSelf _drawingViewSubViewUserInteractionEnabled:nil state:YES];
         }
         else
         {
             [weakSelf _drawingViewSubViewUserInteractionEnabled:nil state:NO];
...
- (void)_createdUI
{
    ...
    placeholderToolView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.3];
    ...
    if (IJSGiPhoneX)
    {
        toolsView.frame = CGRectMake(0, JSScreenHeight - ToolBarMarginBottom, JSScreenWidth - 30, ToolBarMarginBottom + 30);
    }
    ...
    if (_mapView == nil)
    {
        ...
        if (self.mapImageArr)
        {
            ...
            mapView.didClickItemCallBack = ^(NSInteger index, UIImage *indexImage) {
                ...
                [weakSelf _hiddenPlaceholderToolView:YES];
                [weakSelf.toolsView resetButtonState];  // 恢复button的原始状态
            };
...
- (IJSImageMosaicToolView *)mosaicToolView
{
    if (_mosaicToolView == nil)
    {
        ...
        mosaicToolView.typeTwoCallBack = ^(UIButton *button) { // 高斯
            ...
            {
                IJSLodingView *lodingView = [IJSLodingView showLodingViewAddedTo:weakSelf.view title:@"正在处理... ...".lv_localized];
