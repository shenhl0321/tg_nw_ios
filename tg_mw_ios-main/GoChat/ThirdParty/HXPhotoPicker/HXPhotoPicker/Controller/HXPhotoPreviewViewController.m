...
/// 输入广告内容（聊天页面、管理员使用）
@property (nonatomic, strong) UITextView *ADTextInput;
...
- (void)viewDidLoad {
    ...
    [NSNotificationCenter.defaultCenter addObserver:self
                                           selector:@selector(keyboardChanged:) name:UIKeyboardDidHideNotification object:nil];
    [self addGesture];
    
}
- (void)keyboardChanged:(NSNotification *)noti {
    self.keyBoardWillShow = [noti.name isEqualToString:UIKeyboardWillShowNotification];
}
...
- (void)setupUI {
    ...
    if (self.manager.configuration.isCanSendAD) {
        [self.view addSubview:self.ADTextInput];
    }
    self.beforeOrientationIndex = self.currentModelIndex;
    if (self.exteriorPreviewStyle == HXPhotoViewPreViewShowStyleDefault) {
        [self.view addSubview:self.bottomView];
...
- (void)setSubviewAlphaAnimate:(BOOL)animete {
    [_ADTextInput resignFirstResponder];
...
- (void)photoPreviewBottomViewDidDone:(HXPhotoPreviewBottomView *)bottomView {
    ...
    HXPhotoModel *model = self.modelArray[self.currentModelIndex];
    /// 处理管理员发广告
    model.ADContent = _ADTextInput.text;
...
- (BOOL)textViewShouldBeginEditing:(UITextView *)textView {
    HXPhotoEditConfiguration *config = HXPhotoEditConfiguration.new;
    UIColor *white = UIColor.whiteColor;
    config.textColors = @[white];
    config.textFont = [UIFont systemFontOfSize:15];
    HXPhotoEditTextModel *textModel = HXPhotoEditTextModel.new;
    textModel.textColor = white;
    textModel.text = self.ADTextInput.text;
    [HXPhotoEditTextView showEitdTextViewWithConfiguration:config
                                                 textModel:textModel
                                                completion:^(HXPhotoEditTextModel * _Nonnull textModel) {
        self.ADTextInput.text = textModel.text;
    }];
    return NO;
}
#pragma mark - < 懒加载 >
...
- (UITextView *)ADTextInput {
    if (!_ADTextInput) {
        _ADTextInput = [[UITextView alloc] initWithFrame:CGRectMake(0, self.view.hx_h - 50 - hxBottomMargin - 80, self.view.hx_w, 50)];
        _ADTextInput.textAlignment = NSTextAlignmentCenter;
        _ADTextInput.textColor = UIColor.whiteColor;
        _ADTextInput.backgroundColor = UIColor.clearColor;
        _ADTextInput.font = [UIFont systemFontOfSize:14];
        _ADTextInput.zw_placeHolder = @"添加描述";
        _ADTextInput.zw_placeHolderColor = UIColor.lightGrayColor;
        _ADTextInput.delegate = self;
    }
    return _ADTextInput;
}
