...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (model.type == HXPhotoModelMediaTypeCamera) {
        ...
        if (!self.manager.configuration.singleSelected) {
            ...
            for (HXPhotoModel *p in self.previewArray) {
                if (p.type != HXPhotoModelMediaTypeLimit ||
                    p.type != HXPhotoModelMediaTypeCamera) {
                    [previews addObject:p];
                }
            }
            
            NSInteger currentIndex = [self.previewArray indexOfObject:cell.model];
            previewVC.delegate = self;
            previewVC.modelArray = previews;
