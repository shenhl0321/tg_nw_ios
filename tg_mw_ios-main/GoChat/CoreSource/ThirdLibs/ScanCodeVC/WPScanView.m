...
@interface WPScanView ()<AVCaptureMetadataOutputObjectsDelegate,CAAnimationDelegate>
{
...
@property (nonatomic,weak) UILabel         *tishiL;//提示语

@end



@implementation WPScanView
...
- (void)layoutSubviews{
    ...
    if (iPhoneX)
    {
        _upView.frame = CGRectMake((SCREEN_WIDTH - scanViewW) * 0.5, 0, scanViewW, 150);//原来的搞150
    }
    else
    {
        _upView.frame = CGRectMake((SCREEN_WIDTH - scanViewW) * 0.5, 0, scanViewW, 80);//原来的搞150
    }
    //底部view
    ...
    //手电筒
//    _lightBtn.frame = CGRectMake(WIDTH/2 -20, HEIGHT - 180, 40, 40);
    
    //提示语
    _tishiL.frame = CGRectMake(10, SCREEN_HEIGHT - 270, SCREEN_WIDTH-20, 40);//200 变成 270
}
...
- (void)addAnimation{
    ...
    line.hidden = NO;
    CABasicAnimation *animation = [WPScanView moveYTime:2 fromY:[NSNumber numberWithFloat:4] toY:[NSNumber numberWithFloat:self.scanW -2] rep:OPEN_MAX];
