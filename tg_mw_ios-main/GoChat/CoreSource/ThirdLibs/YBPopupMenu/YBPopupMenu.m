...
- (void)show
{
    ...
    [self updateUI];
    CGFloat customViewWidth = _customView.xhq_width;
    [YBMainWindow addSubview:_menuBackView];
    [YBMainWindow addSubview:_customView];
    [YBMainWindow addSubview:self];
    _customView.xhq_width = 0;
    ...
    [self.animationManager displayShowAnimationCompletion:^{
        __strong typeof(weakSelf)self = weakSelf;
        [UIView animateWithDuration:0.3 animations:^{
            self.customView.xhq_width = customViewWidth;
        }];
...
- (void)updateUI
{
    ...
    if (_customView) {
        ...
        if (YBScreenWidth - CGRectGetMinX(self.frame) > CGRectGetWidth(frame)) {
            frame.origin.x = CGRectGetMinX(self.frame);
        } else {
            frame.origin.x = CGRectGetMaxX(self.frame) - CGRectGetWidth(frame);
        }
        frame.origin.y = CGRectGetMinY(self.frame) - CGRectGetHeight(frame) - 10;
...
//        effectView.frame = _menuBackView.bounds;
//        [_menuBackView addSubview:effectView];
    }
