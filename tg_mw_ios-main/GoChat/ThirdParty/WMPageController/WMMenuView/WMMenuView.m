...
- (NSString *)titleFontForState:(WMMenuItemState)state atIndex:(NSInteger)index{
...
- (void)resetBadgeFrame:(NSInteger)index {
    ...
    if (badgeView) {
        ...
        // TODO: 临时手动定位 如果需要精准定位，需要重写item
        badgeFrame.origin.x += frame.origin.x + frame.size.width - badgeFrame.size.width - 13;
        badgeFrame.origin.y = frame.size.height / 2.0f - 10;
...
- (void)addItems {
    ...
    for (int i = 0; i < self.titlesCount; i++) {
        ...
        item.normalFontName = [self titleFontForState:WMMenuItemStateNormal atIndex:i];
        item.selectedFontName = [self titleFontForState:WMMenuItemStateSelected atIndex:i];
