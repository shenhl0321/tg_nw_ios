...
+ (NSString *)dayFromWeekday:(NSDate *)date {
    switch([date weekday]) {
        case 1:
            return @"星期天".lv_localized;
            break;
        case 2:
            return @"星期一".lv_localized;
            break;
        case 3:
            return @"星期二".lv_localized;
            break;
        case 4:
            return @"星期三".lv_localized;
            break;
        case 5:
            return @"星期四".lv_localized;
            break;
        case 6:
            return @"星期五".lv_localized;
            break;
        case 7:
            return @"星期六".lv_localized;
            break;
        default:
            break;
    }
...
+ (NSString *)dayFromWeekday2:(NSDate *)date {
    switch([date weekday]) {
        case 1:
            return @"周日".lv_localized;
            break;
        case 2:
            return @"周一".lv_localized;
            break;
        case 3:
            return @"周二".lv_localized;
            break;
        case 4:
            return @"周三".lv_localized;
            break;
        case 5:
            return @"周四".lv_localized;
            break;
        case 6:
            return @"周五".lv_localized;
            break;
        case 7:
            return @"周六".lv_localized;
            break;
        default:
            break;
    }
...
- (BOOL)isYesterday {
    NSDate *date = [NSDate forward:1 unitType:NSCalendarUnitDay];
    return [self zh_isSameDay:date];
}
...
+ (NSArray *)getDatesWithStartDate:(NSString *)startDate endDate:(NSString *)endDate {
    NSDate *start = [NSDate dateWithString:startDate format:[self ymdFormat]];
    NSDate *end = [NSDate dateWithString:endDate format:[self ymdFormat]];
    return [self getDatesWithStart:start end:end unit:NSCalendarUnitDay format:[self ymdFormat]];
}
...
/// @param endDate 结束时间
/// @param unit 日期类型 （年、月、周、日）
+ (NSArray *)getDatesWithStart:(NSDate *)startDate end:(NSDate *)endDate unit:(NSCalendarUnit)unit format:(NSString *)format {
    NSMutableArray *dates = [NSMutableArray array];
//    NSDateComponents *comps = [[self clsCalendar] components:NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay |  NSCalendarUnitWeekday fromDate:startDate];
    NSDateComponents *tempC = [[NSDateComponents alloc] init];
    NSInteger next = 0;
    NSDate *date = [[self clsCalendar] dateByAddingComponents:tempC toDate:startDate options:0];
    NSComparisonResult result = [date compare:endDate];
    while (result != NSOrderedDescending) {
        [dates addObject:[self stringWithDate:date format:format]];
        next ++;
        [tempC setValue:next forComponent:unit];
        date = [[self clsCalendar] dateByAddingComponents:tempC toDate:startDate options:0];
        result = [date compare:endDate];
    }
    return dates;
}
