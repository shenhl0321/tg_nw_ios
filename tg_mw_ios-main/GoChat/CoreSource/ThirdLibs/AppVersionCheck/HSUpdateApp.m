...
+(void)hs_updateWithAPPID:(NSString *)appId withBundleId:(NSString *)bundelId block:(UpdateBlock)block{
    ...
    if (appId != nil) {
        request = [NSURLRequest requestWithURL:[NSURL URLWithString:[NSString
            stringWithFormat:@"http://itunes.apple.com/cn/lookup?id=%@",appId]]];
            //stringWithFormat:@"http://itunes.apple.com/cn/lookup?id=%@&r=%u",appId,arc4random()]]];
        ChatLog(@"【1】当前为APPID检测，您设置的APPID为:%@  当前版本号为:%@",appId,currentVersion);
        ...
        ChatLog(@"【1】当前为BundelId检测，您设置的bundelId为:%@  当前版本号为:%@",bundelId,currentVersion);
    }else{
        ...
        ChatLog(@"【1】当前为自动检测检测,  当前版本号为:%@",currentVersion);
    }
