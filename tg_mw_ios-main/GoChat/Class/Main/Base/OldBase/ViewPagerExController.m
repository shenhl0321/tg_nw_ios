...
#import "ViewPagerExController.h"
...
#pragma mark - ViewPagerExController
@interface ViewPagerExController () <UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIScrollViewDelegate>
...
@implementation ViewPagerExController
...
//    }
    ...
    if([self.delegate respondsToSelector:@selector(viewPagerWithTabsViewFrameChange:)])
    {
        [self.delegate viewPagerWithTabsViewFrameChange:frame];
    }
...
- (void)setTabWidth:(NSNumber *)tabWidth {
    ...
    if ([tabWidth floatValue] < 0.0)
        tabWidth = [NSNumber numberWithFloat:0.0];
...
- (void)setActiveContentIndex:(NSUInteger)activeContentIndex {
    ...
    __weak ViewPagerExController *weakSelf = self;
