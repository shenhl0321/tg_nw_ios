...
+ (void)_getExportVideoWithAvAssset:(AVAsset *)videoAsset videoComposition:(AVVideoComposition *)videoComposition audioMix:(AVAudioMix *)audioMix timeRange:(CMTimeRange)timeRange completion:(void (^)(NSURL *outputPath, NSError *error, IJSVideoState state))completion cut:(BOOL)isCut
{
    ...
    if ([compatiblePresets containsObject:AVAssetExportPresetHighestQuality])
    {
        ...
        {
            NSError *error = [NSError ijsPhotoSDKVideoActionDescription:@"视频类型暂不支持导出".lv_localized];
            if (completion)
            {
        ...
        [exportSession exportAsynchronouslyWithCompletionHandler:^(void) {
            dispatch_async(dispatch_get_main_queue(), ^{
                switch (exportSession.status)
                {
                    ...
                    case AVAssetExportSessionStatusFailed:
                    {
                        error = [NSError ijsPhotoSDKVideoActionDescription:[NSString stringWithFormat:@"导出失败:%@".lv_localized, exportSession.error]];
                        if (completion)
                        {
                            completion(nil, error, IJSExportSessionStatusFailed);
                        }
        ...
        if (data)
        {
            ...
            else
            {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (completion)
                    {
                        NSError *error = [NSError ijsPhotoSDKImageActionDescription:@"图片写入失败".lv_localized];
